    ...
    methods:{
      getComponentList(page){
        this.$http.post('/components/ComponentList',{
          ...
          this.pageInfo.allpage = Math.ceil( result.data.total/8 );
        }).catch( (error)=> {
          //失败
          console.log(error)
        });
      ...
      viewDetail(id){
        this.$router.push({path:'/Xerath/cpDetail',query:{id:id}})
      },
      addComponent(){
        //添加组件
        this.$router.push({path:'/Xerath/cpEdit'});
      },
      gopage(index){
      ...
      DELItem(id){
        ...
        toast.confirm({
          ...
          onConfirm: function() {
            // 点击“确定”后的回调函数
            _this.$http.post('/components/delete',{id:id})
              .then((result)=>{
                //成功
                toast.message({
                  text:'删除成功！'
                })
                _this.gopage(_this.pageInfo.current);
