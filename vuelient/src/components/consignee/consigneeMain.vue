<template>
  <div class="adminList main">
    <div class="input_box">
      ...
      <input v-model="Admin.usernames" class="myinput" type="text" placeholder="英文名" />
      <input v-if="!editAdminObj" v-model="Admin.useremail" class="myinput" type="email" placeholder="邮箱" />
  ...
  export default {
    ...
    methods:{
      getAdminList(page){
        ...
        this.$http.post('/backlogs/consignees',{
          page:page
      ...
      addAdmin(){ //添加用户
        if(!this.Admin.username || !this.Admin.usernames || !this.Admin.useremail){
          toast.message({
            text:'输入框不能为空！'
          })
          return false;
        }
        this.$http.post('/backlogs/addConsignees',this.Admin)
          .then((result)=>{
            //成功
            this.getAdminList();
            this.emptyAdmin();
